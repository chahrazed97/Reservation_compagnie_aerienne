...
$sql='SELECT id,solde FROM ccp WHERE owner=:owner AND num_compte=:num_compte';
$requete=$bdd->prepare($sql);
$requete->execute(array('owner'=> $owner_card, 'num_compte'=> $num_card));
$donnee=$requete->fetch();
if($donnee==true){
if($donnee['solde']<$_SESSION['total']){
     $dhah='VOTRE SOLDE EST INSUFFISANT POUR EFFECTUER CE PAIEMENT';
     
}else{
//recuperer le solde de la compagnie
$owner_compagnie='air_dorado';
$num_compagnie=1234;
$sql2='SELECT solde FROM ccp WHERE owner=:owner AND num_compte=:num_compte';
...
$stmt0 = $bdd->prepare('UPDATE ccp SET solde=:solde WHERE owner=:owner AND num_compte=:num_card');
$stmt0->bindValue(':solde',$solde_client);
$stmt0->bindValue(':owner',$owner_card);
$stmt0->bindValue(':num_card',$num_card);
$stmt0->execute();
    
$stmt1 = $bdd->prepare('UPDATE ccp SET solde=:solde WHERE owner=:owner AND num_compte=:num_card');
...
$type1='debit';
$type2='credit';
$stmt2=$bdd->prepare('INSERT INTO trans_ccp SET type=:type, compte_id=:compte_id, montant=:montant');
...
$id_compagnie=1;    
$stmt3=$bdd->prepare('INSERT INTO trans_ccp SET type=:type, compte_id=:compte_id, montant=:montant');
...
header('location:info.php');
    ...
    $dhah=  "CE COMPTE N'EXISTE PAS VEUILLEZ VERIFIER VOS INFORMATIONS";
  
}
if(!empty($dhah)){
	header('Location:dhahabia.php?err_d='.$dhah);
}
?>
